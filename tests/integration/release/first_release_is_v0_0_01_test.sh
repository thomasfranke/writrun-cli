...
if [ "$code" -eq 0 ] &&
  ...
  echo "ok    the first release is v0.0.01: stamped, tested, tagged, pushed, published"; pass=$((pass + 1))
else
  echo "FAIL  the first release is v0.0.01: stamped, tested, tagged, pushed, published"
  printf '%s\n' "$out" | sed 's/^/      | /'
  fail=$((fail + 1))
fi
