...
// The harness itself is held against the drawing it reads: a frame that
// reconstructed to nothing would make every assertion below pass for the
// wrong reason.
func TestAFrameReadsBackAsTheLinesItDraws(t *testing.T) {
	got := frame(t, doctorDrawing, "writrun doctor — stage 2 within reach, exit 0")
	if len(got) < 30 {
		t.Fatalf("the frame reconstructed to %d lines:\n%s", len(got), strings.Join(got, "\n"))
	}
	if got[0] != "$ writrun doctor" {
		t.Errorf("first line = %q, want the prompt", got[0])
	}
	if want := "   ✓  git"; !holds(got, want) {
		t.Errorf("the frame holds no %q:\n%s", want, strings.Join(got, "\n"))
	}
}

func holds(rows []string, want string) bool {
	for _, l := range rows {
		if l == want {
			return true
		}
